...
from .setup_guide import setup_payload
...
def _request_base_url(request: Request) -> str:
    ...
    host = (
        ...
        or f"localhost:{os.getenv('PORT') or os.getenv('GATEWAY_PORT') or '3100'}"
    ).split(",")[0].strip()
...
async def api_setup(request: Request) -> JSONResponse:
    """Slack + WxO setup guide, env checklist, example agent YAMLs (no secret values)."""
    denied = _deny_unless_admin(request)
    if denied is not None:
        return denied  # type: ignore[return-value]
    return JSONResponse(setup_payload(base_url=_request_base_url(request)))


@mcp.custom_route("/api/examples/agents", methods=["GET"])
async def api_examples_agents(request: Request) -> JSONResponse:
    """Example Orchestrate agent YAML files shipped with the gateway."""
    denied = _deny_unless_admin(request)
    if denied is not None:
        return denied  # type: ignore[return-value]
    data = setup_payload(base_url=_request_base_url(request))
...
def _resolve_transport() -> str:
    """stdio | streamable-http | sse — IDEs use stdio or remote URL to /mcp."""
    raw = (os.getenv("GATEWAY_TRANSPORT") or "").strip().lower()
    if raw in ("stdio", "streamable-http", "sse", "http"):
        return "streamable-http" if raw == "http" else raw
    # CLI: python -m slack_mcp_gateway --stdio
...
def main() -> None:
    ...
    # Poller: on by default for HTTP hosts; off for stdio IDE clients unless opted in.
    enable_poller = os.getenv("GATEWAY_ENABLE_POLLER", "").strip().lower()
    if enable_poller in ("1", "true", "yes"):
        start_poller()
    elif enable_poller in ("0", "false", "no"):
        log.info("Poller disabled (GATEWAY_ENABLE_POLLER=%s)", enable_poller)
    elif transport == "stdio":
        log.info("Poller off in stdio mode (set GATEWAY_ENABLE_POLLER=1 to enable)")
    else:
        start_poller()

    if transport == "stdio":
        log.info("Starting Slack↔WxO MCP Gateway (stdio) for Cursor / VS Code / Bob / Antigravity")
    ...
    if transport == "sse":
        log.info("Starting Slack↔WxO MCP Gateway on %s:%s (SSE /sse, UI /)", host, port)
